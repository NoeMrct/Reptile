...
const REL = {
  father: t('pedigree.rel.father'),
  mother: t('pedigree.rel.mother'),
  paternalGrandfather: t('pedigree.rel.paternalGrandfather'),
  paternalGrandmother: t('pedigree.rel.paternalGrandmother'),
  maternalGrandfather: t('pedigree.rel.maternalGrandfather'),
  maternalGrandmother: t('pedigree.rel.maternalGrandmother'),
};

const displaySex = (sex?: Snake['sex']) => {
...
const PedigreeTab: React.FC = () => {
  ...
  const listAncestors = (snake: Snake, generations = 3): AncestorRow[] => {
    const rows: AncestorRow[] = [
      { snake, generation: 0, relation: t('pedigree.rel.individual'), parentMaleId: snake.parentMaleId, parentFemaleId: snake.parentFemaleId },
    ];
    ...
    const walk = (
      ...
      const rel = (() => {
        ...
        const side = sidePath.map((p) => (p === 'paternal' ? t('pedigree.rel.side.paternal') : t('pedigree.rel.side.maternal'))).join('/');
        if (gen === 3) return t('pedigree.rel.greatGrandparent', { side });
        return t('pedigree.rel.nthGreatGrandparent', { n: gen - 1, side });
      })();
      ...
      {s ? (
        <>
          ...
          <div className="text-xs text-gray-500 space-y-1">
            <p>{displaySex(s.sex)}</p>
            {s.birthDate && <p>{new Date(s.birthDate).getFullYear()}</p>}
          </div>
        </>
      ) : (
        <div className="text-gray-400 text-sm">{t('common.unknown')}</div>
      )}
    </div>
  );
        ...
        {cols.map((genRows, gen) => (
          <div key={gen} className="min-w-[280px]">
            <div className="sticky top-0 z-10 bg-gray-100/80 backdrop-blur border rounded-md px-3 py-1.5 text-sm font-semibold text-gray-700 mb-3">
              {t('pedigree.generation')} {gen}
            </div>
            <div className="flex flex-col gap-4">
              {genRows.map((r, i) => (
                <Card key={`${gen}-${i}-${r.snake?.id || "x"}`} s={r.snake} label={gen === 0 ? t('pedigree.rel.individual') : r.relation} />
              ))}
            </div>
          </div>
        ))}
    ...
    return (
      <div className="space-y-6">
        {Object.keys(perGen)
          ...
          .map((gen) => (
            <div key={gen}>
              <div className="text-sm font-semibold text-gray-700 mb-3">{t('pedigree.generation')} {gen}</div>
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
        <thead className="bg-gray-50">
          <tr>
            <th className="text-left text-xs font-semibold text-gray-600 px-4 py-2">{t('pedigree.table.generation')}</th>
            <th className="text-left text-xs font-semibold text-gray-600 px-4 py-2">{t('pedigree.table.relation')}</th>
            <th className="text-left text-xs font-semibold text-gray-600 px-4 py-2">{t('pedigree.table.name')}</th>
            <th className="text-left text-xs font-semibold text-gray-600 px-4 py-2">{t('pedigree.table.sex')}</th>
            <th className="text-left text-xs font-semibold text-gray-600 px-4 py-2">{t('pedigree.table.morph')}</th>
            <th className="text-left text-xs font-semibold text-gray-600 px-4 py-2">{t('pedigree.table.birthYear')}</th>
          </tr>
        </thead>
        <tbody>
          {ancestorRows
            ...
            .map((r, i) => (
              <tr key={`${r.generation}-${i}-${r.snake?.id || "x"}`} className="border-t">
                ...
                <td className="px-4 py-2 text-sm text-gray-900">{r.snake?.name || t('common.unknown')}</td>
                <td className="px-4 py-2 text-sm text-gray-700">{displaySex(r.snake?.sex)}</td>
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const drawHeader = () => {
        ...
        const title = t('pedigree.pdf.title');
        ...
        const subtitle = `${t('brand.name')} - ${t('breeding.title')}`;
      ...
      const drawMeta = () => {
        ...
        pdf.setFontSize(12);
        pdf.text(t('pedigree.pdf.infoSection'), left, y);

        y += 8;
        pdf.setFont("helvetica", "normal");
        pdf.setTextColor(75, 85, 99);
        const lines: Array<[string, string]> = [
          [t('pedigree.pdf.field.individual'), selectedSnake.name],
          [t('pedigree.pdf.field.species'), selectedSnake.species || "—"],
          [t('pedigree.pdf.field.morph'), selectedSnake.morph || "—"],
          [t('pedigree.pdf.field.sex'), displaySex(selectedSnake.sex)],
          [t('pedigree.pdf.field.generationsShown'), String(maxGen)],
          [t('pedigree.pdf.field.exportedOn'), new Date().toLocaleDateString()],
        ];
        const labelWidth = 55;
      ...
      const drawFooter = () => {
        ...
        const footer = t('pedigree.pdf.footer', { brand: t('brand.name'), date: new Date().toLocaleDateString() });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-xl font-bold text-gray-900">{t('pedigree.title')}</h3>
          <p className="text-gray-600 mt-1">{t('pedigree.subtitle')}</p>
        </div>
        <div className="flex items-center">
          ...
          <button
            ...
            title={t('pedigree.export.tooltip')}
          >
            <Download className="h-4 w-4" />
            {exporting ? t('pedigree.export.exporting') : t('common.exportPdf')}
          </button>
        </div>
      </div>

      <div className="flex flex-wrap items-center gap-4">
        <label className="text-sm font-medium text-gray-700">{t('pedigree.selectLabel')}</label>
        <select
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            title={t('pedigree.viewTitle.vertical')}
          >
            <ListTree className="w-4 h-4" /> {t('pedigree.view.vertical')}
          </button>
          <button
            ...
            title={t('pedigree.viewTitle.horizontal')}
          >
            <Columns className="w-4 h-4" /> {t('pedigree.view.horizontal')}
          </button>
          <button
            ...
            title={t('pedigree.viewTitle.grid')}
          >
            <LayoutGrid className="w-4 h-4" /> {t('pedigree.view.grid')}
          </button>
          <button
            ...
            title={t('pedigree.viewTitle.table')}
          >
            <Table className="w-4 h-4" /> {t('pedigree.view.table')}
          </button>
          <button
            ...
            title={t('pedigree.viewTitle.compact')}
          >
            <Rows className="w-4 h-4" /> {t('pedigree.view.compact')}
          </button>
        </div>

        <div className="flex items-center gap-2 ml-auto">
          <span className="text-sm text-gray-700">{t('pedigree.depthLabel')}</span>
          <select
            value={maxGen}
      ...
      {selectedSnake ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-600">{t('pedigree.empty')}</p>
        </div>
      )}

      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h4 className="font-semibold text-blue-900 mb-2">{t('pedigree.legend.title')}</h4>
        <div className="flex flex-wrap gap-6 text-sm">
          <div className="flex items-center">
            <div className="w-4 h-4 border-2 border-blue-500 rounded mr-2" />
            <span className="text-blue-900">{t('sex.male')}</span>
          </div>
          <div className="flex items-center">
            <div className="w-4 h-4 border-2 border-pink-500 rounded mr-2" />
            <span className="text-blue-900">{t('sex.female')}</span>
          </div>
          <div className="flex items-center">
            <div className="w-4 h-4 border-2 border-gray-300 rounded mr-2" />
            <span className="text-blue-900">{t('common.unknown')}</span>
          </div>
        </div>
      </div>
      ...
      {showParentsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
            <h4 className="text-lg font-semibold mb-4">{t('pedigree.parents.title')}</h4>

            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('pedigree.parents.fatherLabel')}</label>
                <select
                  className="w-full border rounded-lg px-3 py-2"
                  value={fatherId}
                  onChange={(e) => setFatherId(e.target.value)}
                >
                  <option value="">{t('pedigree.parents.noneMale')}</option>
                  {males.map((m) => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('pedigree.parents.motherLabel')}</label>
                <select
                  className="w-full border rounded-lg px-3 py-2"
                  value={motherId}
                  onChange={(e) => setMotherId(e.target.value)}
                >
                  <option value="">{t('pedigree.parents.noneFemale')}</option>
                  {females.map((f) => (
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button className="px-4 py-2 rounded-lg border" onClick={() => setShowParentsModal(false)}>{t('common.cancel')}</button>
              <button
